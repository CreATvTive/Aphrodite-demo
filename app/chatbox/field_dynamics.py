...
class FieldDynamics:
    ...
    __slots__ = ("_registrations", "_states", "_rngs", "_tick", "_rng_seeds")

    def __init__(
        self,
        registry: Sequence[DimensionRegistration] | None = None,
        *,
        rng_factory: GaussianRngFactory | None = None,
    ) -> None:
        ...
        try:
            rngs = tuple(factory.create(dim_id) for dim_id in dim_ids)
        except DynamicsContractError:
            raise
        except Exception as exc:
            raise DynamicsContractError(
                "rng_factory_failure", str(exc), stage="rng_factory"
            ) from exc
    ...
    def tick(self) -> TickObservation:
        ...
        candidate_seeds: list[int | None] = list(self._rng_seeds)
        next_tick = self._tick + 1
        for dim_index, (state, rng) in enumerate(zip(self._states, self._rngs)):
            registration = state.registration
            try:
            ...
            if candidate_seeds[dim_index] is not None and draw.seed != candidate_seeds[dim_index]:
                raise InvalidRngDrawError(
                    "invalid_rng_draw",
            ...
            after_ou = ou_rho * state.ou_acceleration + ou_innovation_scale * draw.value
